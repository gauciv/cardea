...
import { LayoutDashboard, Server, Settings, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 h-full bg-slate-900/95 backdrop-blur-sm border-r border-slate-800 z-50 flex flex-col transition-all duration-200 ease-out ${
        ...
        {/* User (clickable to profile) & Logout */}
        <div className="border-t border-slate-800/50 p-2 space-y-1">
          {user && (
            <button
              onClick={() => { navigate('/profile'); if (window.innerWidth < 1024) setIsOpen(false); }}
              className={`w-full flex items-center gap-2 px-2.5 py-2 rounded-lg hover:bg-slate-800 transition-colors group ${isOpen ? '' : 'justify-center'} ${currentPath === '/profile' ? 'bg-cyan-500/10' : ''}`}
            >
              ...
              <div className={`transition-opacity duration-200 text-left ${isOpen ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}>
                <p className={`text-[11px] font-medium truncate ${currentPath === '/profile' ? 'text-cyan-400' : 'text-white'}`}>{displayName}</p>
